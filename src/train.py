...
def _pick_optional_float(
    ...
    if env_key:
        env_val = _env_get(env_key)
        return None if env_val is None else float(env_val)
    return None
...
def _pick_optional_str(
    ...
    if env_key:
        env_val = _env_get(env_key)
        return env_val if env_val is not None else None
    return None
...
def parse_args(argv=None):
    ...
    ap.set_defaults(resume=None)
    ap.add_argument("--eps-mode", type=str, choices=["normal", "freeze", "override"], default=None)
    ap.add_argument("--eps-override", type=float, default=None)
    ap.add_argument("--alpha-override", type=float, default=None)
    ap.add_argument("--mul-real", type=float, default=None)
    ap.add_argument("--mul-sem", type=float, default=None)
    ap.add_argument("--mul-cf", type=float, default=None)
    ap.add_argument("--mul-mono", type=float, default=None)
    ap.add_argument("--mul-cos", type=float, default=None)
    ap.add_argument("--mul-ent", type=float, default=None)
    ap.add_argument("--mul-env", type=float, default=None)
    ap.add_argument("--mul-self", type=float, default=None)
    ap.add_argument("--self-target", type=float, default=None)
    ap.add_argument("--island-every", type=int, default=None)
    ap.add_argument("--island-max-samples", type=int, default=None)
    ap.add_argument("--tail-start", type=int, default=None)
    ap.add_argument("--tail-every", type=int, default=None)
    ap.add_argument("--lr-table", type=str, default=None)
...
def main(argv=None):
    ...
    run_id = _pick_optional_str("run_id", args.run_id, cfg, "RUN_ID")
    run_dir_arg = _pick_optional_str("run_dir", args.run_dir, cfg, "RUN_DIR")
    run_dir, run_id = resolve_run_dir(run_dir_arg, run_id)
    paths = build_run_paths(run_dir, run_id)
    ensure_run_layout(paths)

    seed = _pick_value("seed", args.seed, cfg, "SEED", 0, int)
    device = _pick_value("device", args.device, cfg, "DEVICE", C.device, str)

    n = _pick_value("n", args.n, cfg, "N", C.N, int)
    epochs = _pick_value("epochs", args.epochs, cfg, "EPOCHS", C.EPOCHS, int)
    lr = _pick_value("lr", args.lr, cfg, "LR", C.LR, float)
    log_every = _pick_value("log_every", args.log_every, cfg, "LOG_EVERY", C.LOG_EVERY, int)
    ...
    if resume is None:
        if "resume" in cfg:
            ...
            resume = (_env_get("RESUME") == "1")
    resume = bool(resume)

    eps_mode = _pick_value("eps_mode", args.eps_mode, cfg, "EPS_MODE", "normal", str).lower()
    if eps_mode not in {"normal", "freeze", "override"}:
        raise ValueError(f"Invalid eps_mode={eps_mode}")
    eps_override_val = _pick_optional_float("eps_override", args.eps_override, cfg, "EPS_OVERRIDE")
    alpha_override_val = _pick_optional_float("alpha_override", args.alpha_override, cfg, "ALPHA_OVERRIDE")

    mul_real = _pick_value("mul_real", args.mul_real, cfg, "MUL_REAL", 1.0, float)
    mul_sem  = _pick_value("mul_sem",  args.mul_sem,  cfg, "MUL_SEM",  1.0, float)
    mul_cf   = _pick_value("mul_cf",   args.mul_cf,   cfg, "MUL_CF",   1.0, float)
    mul_mono = _pick_value("mul_mono", args.mul_mono, cfg, "MUL_MONO", 1.0, float)
    mul_cos  = _pick_value("mul_cos",  args.mul_cos,  cfg, "MUL_COS",  1.0, float)
    mul_ent  = _pick_value("mul_ent",  args.mul_ent,  cfg, "MUL_ENT",  1.0, float)
    mul_env  = _pick_value("mul_env",  args.mul_env,  cfg, "MUL_ENV",  1.0, float)
    mul_self = _pick_value("mul_self", args.mul_self, cfg, "MUL_SELF", 1.0, float)

    self_target_val = _pick_value(
        "self_target", args.self_target, cfg, "SELF_TARGET_OVERRIDE", C.SELF_TARGET, float
    )

    island_every = _pick_value("island_every", args.island_every, cfg, "ISLAND_EVERY", log_every, int)
    island_max_samples = _pick_value(
        "island_max_samples", args.island_max_samples, cfg, "ISLAND_MAX_SAMPLES", 512, int
    )

    tail_start = _pick_value(
        "tail_start", args.tail_start, cfg, "TAIL_START", max(0, epochs - 6 * log_every), int
    )
    tail_every = _pick_value(
        "tail_every", args.tail_every, cfg, "TAIL_EVERY", max(1, log_every // 4), int
    )

    lr_table = _pick_optional_str("lr_table", args.lr_table, cfg, "LR_TABLE")
    if not lr_table:
        lr_table = str(paths.run_dir / "lr_mult_by_epoch.csv")

    C.N = int(n)
    C.EPOCHS = int(epochs)
    C.LR = float(lr)
    C.LOG_EVERY = int(log_every)
    C.device = str(device)
    ...
    env_keys = [
        "SEED", "RUN_ID", "RUN_DIR", "RESUME",
        "EPS_MODE", "EPS_OVERRIDE", "ALPHA_OVERRIDE",
        "MUL_REAL", "MUL_SEM", "MUL_CF", "MUL_MONO",
        "MUL_COS", "MUL_ENT", "MUL_ENV", "MUL_SELF",
        "SELF_TARGET_OVERRIDE", "ISLAND_EVERY", "ISLAND_MAX_SAMPLES",
        "TAIL_START", "TAIL_EVERY", "LR_TABLE",
    ]
    meta = repro.build_metadata(sys.argv, extra={"env": repro.collect_env(env_keys)})
    repro.write_json(paths.meta_path, meta)

    config_snapshot = {
        "run": {"run_id": run_id, "run_dir": str(paths.run_dir)},
        "seed": int(seed),
        "device": str(C.device),
        "data": {"n": int(C.N), "train_ratio": float(C.TRAIN_RATIO)},
        "model": {"d_model": int(C.D_MODEL), "n_heads": int(C.N_HEADS)},
        "train": {
            "epochs": int(C.EPOCHS),
            "lr": float(C.LR),
            "log_every": int(C.LOG_EVERY),
            "tail_start": int(tail_start),
            "tail_every": int(tail_every),
            "island_every": int(island_every),
            "island_max_samples": int(island_max_samples),
            "resume": bool(resume),
        },
        "eps": {
            "mode": eps_mode,
            "eps_override": (None if eps_override_val is None else float(eps_override_val)),
            "alpha_override": (None if alpha_override_val is None else float(alpha_override_val)),
            "alpha_k": float(C.ALPHA_K),
        },
        "loss_multipliers": {
    ...
    repro.write_json(paths.config_path, config_snapshot)
    ...
    for _name, _val in {
        ...
        "mul_env": mul_env,
        "mul_self": mul_self,
        "self_target": self_target_val,
    }.items():
        if not math.isfinite(float(_val)):
            raise ValueError(f"Invalid {_name}={_val}")
    print(
        "[MUL] "
    ...
    print(f"[TAIL] start={tail_start} every={tail_every} (LOG_EVERY={log_every})")

    # ---- Output paths (RUN_DIR based) ----
    LOG_PATH = paths.alpha_log
    ...
    if RESUME:
        ...
        if last_alpha is not None and alpha_override_val is None:
    ...
    DEFAULT_LR_TABLE = str(paths.run_dir / "lr_mult_by_epoch.csv")
    LR_TABLE = lr_table or DEFAULT_LR_TABLE
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for pg in opt.param_groups:
            pg["lr"] = lr_now
        if should_log_epoch(epoch, log_every=log_every, tail_start=tail_start, tail_every=tail_every, epochs=epochs):
        ...
        if should_log_epoch(epoch, log_every=log_every, tail_start=tail_start, tail_every=tail_every, epochs=epochs):
            ...
            if epoch % island_every == 0:
                path = save_island_snapshot(
                    ...
                    device=C.device,
                    out_dir=ISLAND_DIR,
                    max_samples=island_max_samples,
                )
