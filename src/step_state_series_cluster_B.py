...
def main():
    ...
    ap.add_argument("--thr", type=str, default="/content/threshold_timeseries_with_events.csv")
    ap.add_argument("--env", type=str, default="/content/island_env_error.csv")
    ap.add_argument("--prof", type=str, default="/content/island_profile.csv")
    ap.add_argument("--out", type=str, default="stepB_state_series_clustering")
    ...
    prof = pd.read_csv(args.prof)

    # --- build epoch-level table (NO island join for thr) ---
    state_epoch = build_epoch_state_table(thr, env, prof)
    state_epoch.to_csv(os.path.join(args.out, "state_epoch_table.csv"), index=False)
    ...
    # save matrices
    pd.DataFrame(X_traj).to_csv(os.path.join(args.out, "trajectories_matrix.csv"), index=False)
    meta = meta.copy()
    meta["cluster"] = labels
    meta.to_csv(os.path.join(args.out, "trajectory_cluster_assignments.csv"), index=False)
    ...
    summ = (meta.groupby("cluster")
            ...
            .reset_index()
            .sort_values("cluster"))
    summ.to_csv(os.path.join(args.out, "cluster_summary.csv"), index=False)
    ...
    plt.ylabel("PC2")
    plt.tight_layout()
    plt.savefig(os.path.join(args.out, "pca_scatter.png"), dpi=160)
    plt.close()
    ...
    plt.ylabel("cluster")
    plt.tight_layout()
    plt.savefig(os.path.join(args.out, "cluster_centroid_heatmap.png"), dpi=160)
    plt.close()

    # optional: silhouette scores dump
    if sil_scores is not None:
        pd.DataFrame(sil_scores, columns=["k", "silhouette"]).to_csv(
            os.path.join(args.out, "silhouette_scores.csv"), index=False
        )
