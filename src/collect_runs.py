...
def find_alpha_logs() -> List[str]:
    root = repo_root()
    patterns = [str(root / "runs" / "*" / "alpha_log.csv")]
    paths: List[str] = []
    for pat in patterns:
...
def run_detect(csv_path: str, json_out: str,
...
def main(argv=None):
    ...
    if not logs:
        raise RuntimeError("No alpha_log.csv found under runs/*")
    ...
    for csv_path in logs:
        ...
        if args.recompute or (not os.path.exists(json_out)):
            ...
            payload = run_detect(csv_path, json_out, extra_args=args.extra)
        else:
