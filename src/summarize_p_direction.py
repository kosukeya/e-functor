...
#     --include_p_series
#
# If --runs is omitted, it will auto-glob runs/*/alpha_log.csv
...
from detect_critical import parse_args as detect_parse_args, run_detect_critical
from run_utils import repo_root


def run_detect(csv_path: str, json_out: str, include_p_series: bool) -> Dict:
    argv = [csv_path, "--json_out", json_out]
    if include_p_series:
        argv.append("--include_p_series")
    args = detect_parse_args(argv)
    args.csv_path = csv_path
    args.json_out = json_out
    return run_detect_critical(args)
...
def find_default_runs() -> List[str]:
    root = repo_root()
    pats = [str(root / "runs" / "*" / "alpha_log.csv")]
    out: List[str] = []
    for pat in pats:
...
def main(argv=None):
    ...
    if args.detect and args.detect != "detect_critical.py":
        print("[WARN] --detect is ignored; using in-process detect_critical.")
    ...
    for csv_path in runs:
        ...
        json_out = os.path.join(run_dir, "collapse", "critical.json")

        if args.recompute or (not os.path.exists(json_out)):
            print(f"[sum] running detect: {csv_path}")
            payload = run_detect(csv_path, json_out, include_p_series=args.include_p_series)
        else:
