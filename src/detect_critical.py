...
def parse_args(argv=None):
    ...
    ap.add_argument("csv_path", type=str, nargs="?", default=None, help="Path to alpha_log.csv")
    ap.add_argument("--csv", dest="csv_path", type=str, default=None, help="Path to alpha_log.csv")
    ...
    ap.add_argument("--json_out", dest="json_out", type=str, default=None,
                    help="If set, write result JSON to this path.")
    ap.add_argument("--json-out", dest="json_out", type=str, default=None,
                    help="If set, write result JSON to this path.")
...
def run_detect_critical(args):
    ...
    if not csv_path:
        if args.run_dir or args.run_id:
            ...
            csv_path = str(paths.alpha_log)
            if args.json_out is None:
                args.json_out = str(paths.collapse_dir / "critical.json")
        else:
            csv_path = None
    if not csv_path:
        raise ValueError("csv_path is required (or specify --run-dir/--run-id)")
...
def detect_critical_from_csv(csv_path: str, **kwargs):
    args = parse_args([])
    args.csv_path = csv_path
